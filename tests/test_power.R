...
# the four scenarios draw from four different seed steps
stopifnot(length(unique(c(seed_for("power"), seed_for("power_6"), seed_for("power_10"),
                          seed_for("power_12")))) == 4L)
...
of8 <- file.path("outputs", "08_power",
                 c("mde.csv", "placebo_draws.csv", "mde_10states.csv",
                   ...
                   "mde_6states.csv", "placebo_draws_6states.csv"))
if (all(file.exists(of8))) {
  ...
  # four scenarios, three primary gaps each, on the primary (unbalanced) step 5 panel;
  # the projection belongs to the three drawn-year files
  stopifnot(nrow(s8) == 12L,
            setequal(s8$scenario, c("observed", "six_state", "ten_state", "twelve_state")),
  ...
  stopifnot(all(m8$scenario == "observed"), all(m6$scenario == "six_state"),
            all(m10$scenario == "ten_state"), all(m12$scenario == "twelve_state"),
            !any(PROJ %in% names(m8)), all(PROJ %in% names(m10)),
            identical(names(m12), names(m10)),          # same columns as the ten-state file
            identical(names(m6), names(m10)))
  stopifnot(six8 == 6L, ten8 == 10L, twelve8 == 12L, all(m6$treated_states == six8),
            all(m10$treated_states == ten8),
            all(m12$treated_states == twelve8), all(m8$treated_states >= 1),
            all(m8$treated_states < six8))
  # the registered power calculation is the six-state run: six is the number of treated
  # states the event table carries with post-reform data by the registration end year
  # 2021. The observed-cohort, ten- and twelve-state runs are sensitivity runs.
  stopifnot(set8("registered_scenario") == "six_state",
            set8("seed_step_six_state") == "power_6",
            set8("seed_step_twelve_state") == "power_12",
            all(m6$design_treated_states == six8),       # 6 matches the event table count
            all(m10$design_treated_states == six8), all(m12$design_treated_states == six8))

  # each scenario: the MDE is the one its own saved draws give
  for (part in list(list(m = m8, d = d8), list(m = m6, d = d6), list(m = m10, d = d10),
                    list(m = m12, d = d12))) {
  ...
  # more placebo-treated states buy a tighter placebo distribution: 2 < 6 < 10 < 12. The
  ...
  k6 <- match(m8$gap, m6$gap); k10 <- match(m8$gap, m10$gap); k12 <- match(m8$gap, m12$gap)
  stopifnot(!anyNA(k6), !anyNA(k10), !anyNA(k12),
            all(m6$crit_value[k6] < m8$crit_value), all(m6$mde[k6] < m8$mde),
            all(m10$crit_value[k10] < m6$crit_value[k6]), all(m10$mde[k10] < m6$mde[k6]),
  ...
  for (mm in list(m6, m10, m12)) {
    ...
    stopifnot(all(mm$design_treated_states >= 1), all(mm$design_mean_post_years > 0),
              ...
              # with the end year at 2021 the registered window is no longer longer than the
              # placebo one by construction, so the direction of the projection follows the
              # state-year ratio it is built from
              all((mm$mde_projection < mm$mde) ==
                    (mm$placebo_post_state_years < mm$design_post_state_years)),
              all(grepl("projection", mm$projection_basis)),
              all(grepl("not a power calculation", mm$projection_basis)))
  }
  # the projection end year is the registration end year, and the dropped end year is
  # out of the count
  stopifnot(set8("projection_end_year") == "2021", set8("projection_dropped_years") == "2020")

  # the settings file records every scenario's seed and the counts this run used
  stopifnot(all(c("placebo_reps", "quick_run", "seed_step_observed", "seed_step_six_state",
                  "seed_step_ten_state", "seed_step_twelve_state", "six_state_count",
                  "ten_state_count", "twelve_state_count",
